...
{
    public class GreatSword : Component, IUpdateable, ILoadable, IAnimateable, ICollisionStay, ICollisionEnter, ICollisionExit
    {
        Vector2 mousePos;
        Vector2 playerPos;
        ...
        public GreatSword(GameObject gameObject, Player player) : base(gameObject)
        {
            this.player = player;
        ...
        public void Update()
        {
            if (GameWorld.Instance.GetIsOnGroundGreatSword)
            {
            ...
            else
            {
                ...
                }
                {
                    ...
                    rotation = (float)Math.Atan2(cursorDirection.Y, cursorDirection.X) + (float)(Math.PI * 0.65f);
